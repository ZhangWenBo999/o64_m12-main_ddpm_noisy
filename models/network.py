...
class Network(BaseNetwork):
    ...
    @torch.no_grad()
    def restoration_test(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8):
        b, *_ = y_cond.shape
    ...
    @torch.no_grad()
    def restoration_eval(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8):
        b, *_ = y_cond.shape
        ...
        sample_count = 0
        for i, j in tqdm((zip(reversed(seq), reversed(seq_next))), desc='sampling loop time step', total=self.skip_step):
            sample_count += 1
            ...
            mean_eps = (
                (atm1.sqrt() * beta_t) * y_0_hat + ((1 - beta_t).sqrt() * (1 - atm1)) * y_t
            ) / (1.0 - at)
            mean = mean_eps
